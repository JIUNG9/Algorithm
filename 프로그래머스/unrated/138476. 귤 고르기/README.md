...
### Approach
처음에는 정렬을 해서 각 숫자별로 연속된 숫자를 빼면서 입력값이 0이 되는 순간을 찾으려고 했다. 하지만 이 경우 가장 많은 연속된 숫자를 먼저 카운트하지 못 하기 때문에 최적의해가 아니게 된다. 따라서 연속된 숫자가 많은 순으로 정렬하기위해 Map Interface를 활용하고 값을 기준으로 오름차순 정렬하면 가장 적은 개수의 조합이 나오게된다.

### 성능 요약

메모리: 91.9 MB, 시간: 54.93 ms
