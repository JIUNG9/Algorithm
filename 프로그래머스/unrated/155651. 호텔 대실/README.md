...
### Approach
비교를 입실 시간으로 시작하고, Priority Quque를 퇴실시간 기준으로 가장 빠르게 퇴실하는 사람을 기준으로 잡는다면, 새로 입실 하는 사람이 가장 빠른 퇴실 시간보다 빠르게 입실한다면 방을 추가, 아니라면 이전에 있던 방을 재사용이 가능하다. 청소시간이 10분이 있기 때문에 String값을 int 시간으로 변환할 때 10분을 추가한다. 분을 다룰 때는 60분이 넘어가는 경우를 고려해서 int time으로 변환한다.
