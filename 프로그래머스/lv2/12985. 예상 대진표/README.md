...
### Approach
무조건 이긴다는 조건을 가지므로 A와 B의 입력값은 /2를 진행하면서 서로가 대진표에서 만날 때까지의 count를 세면됩니다. 만약 홀 수번째의 팀이라면 n/2 + 1번째의 팀이 되고 해당 루프의 종료조건은 A와 B를 각각 작은 수와 큰 수로 정렬해놓은 상태이기에 B-A가 1인 경우와 A가 홀수인 경우 B가 짝수인 경우에 해당 대진이 이뤄지는 것입니다. B-A가 1인 경우에 A가 2번이고 B가 3번일 경우 각각 다른 상대와 경기를 진행하므로 A가 홀수이고 B가 짝수인 경우를 조건에 추가해줘야합니다.
### 성능 요약
