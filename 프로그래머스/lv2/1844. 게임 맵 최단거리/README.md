...
# Approach
1. 최단거리를 BFS로 접근한다.
2. 도달하지 못 하는 경우는 가중치의 배열을 초기화하였을 때 도착지점의 가중치는 0이므로 도착지점의 가중치가 0일 경우에 -1를 리턴한다.
3. 방문할 수 있는 경우와 방문하지 못 하는 경우를 생각하여 구현한다.
# Mistake
1. 범위와 방문하지 못 하는 경우를 동시에 체크를 해서 범위를 초과해서 검색하는 경우가 생겼다. 이를 분리해서 구현하였다.
# Implementation
1. Queue를 통해 BFS를 구현한다.
2. 상하좌우를 확인하기 위해 dx,dy 배열을 static으로 선언한다.
3. 가중치 배열은 visited배열을 int로 생성하여 가중치를 확인하여 해당 위치를 이미 방문했는지 확인한다.

### 성능 요약

메모리: 53.1 MB, 시간: 4.27 ms
